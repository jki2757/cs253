...
import edu.emory.cs.sort.hybrid.HybridSortKim;
...
public class HybridSortTest {
    ...
    public void testAccuracy() {
        ...
        HybridSort<Integer> mine = new HybridSortKim<>();  // TODO: replace with your class
        ...
        for (int row = 10; row <= 20; row++)
            for (int col = 10; col <= 20; col++)
                for (int i = 0; i < 100; i++)
                    testAccuracy(randomInput(row, col, 0.25), gold, mine);// 100?**
    }
...
//    }
    ...
    private Integer[][] randomInput(int row, int col, double ratio) {
        ...
        for (int i = 0; i < row; i++)
            input[i] = randomArray(col, ratio);// which array?

        return input;
    }

    private Integer[] randomArray(int size, double ratio) {//which one produces natural ascending
